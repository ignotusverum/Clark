...
class ProgressDimentionCell: UICollectionViewCell, DividerCellProtocol, SimpleTitleCellProtocol {
    
    /// Divider view
    lazy var dividerView: UIView = self.generateDividerView()
    
    /// Dimention
    var dimention: ProgressDimention! {
        didSet {
    ...
    class func calculateHeight(_ progressDimention: ProgressDimention)-> CGFloat {
        ...
        for dimention in progressDimention.descriptions {
            ...
            let copy = dimention
            textHeight += copy.heightWithConstrainedWidth(screenWidth, font: UIFont.defaultFont(style: .light, size: 16))
        }
        
        return progressDimention.descriptions.count > 0 ? titleHeight + textHeight + 24 : 0
    }
}
