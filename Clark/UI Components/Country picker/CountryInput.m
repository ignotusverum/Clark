...
- (void) awakeFromNib
{
    ...
    self.countryPickerContainer = [[UIView alloc] initWithFrame:(CGRect){0, 0, 320, COUNTRYINPUTHEIGHT}];
    ...
    if (self.countryDisplay == nil) {
        ...
        if (IS_IPHONE && SCREEN_MAX_LENGTH <= 736.0f) {
            yPosition = 20.0;
        }
...
- (void) updateStyle:(BOOL)isLight {
    ...
    if (!isLight) {
        ...
        self.countryPickerContainer.backgroundColor = [UIColor blackColor];
        self.countryPicker.backgroundColor = [UIColor blackColor];
...
- (void) presentPicker
{
    if (self.viewForPicker) {
        ...
        startFrame.size.height = COUNTRYINPUTHEIGHT;
        
        CGRect endFrame = self.viewForPicker.frame;
        endFrame.origin.x = 0;
        endFrame.origin.y = endFrame.size.height - COUNTRYINPUTHEIGHT;
        endFrame.size.height = COUNTRYINPUTHEIGHT;
